...
public class GoToAddOrgConfirmCommand extends AuthCommand {
    ...
    protected Router process(HttpServletRequest req) {
        ...
        if (accountInfo == null || accountInfo.getStatus().getId() != ACTIVE_STATUS) {
            ...
            router = new Router(PagePath.GO_TO_ADD_ORG_COMMAND, RouterType.REDIRECT);
        } else {
            req.setAttribute(RequestAttribute.ACCOUNT_INFO, accountInfo);
            req.setAttribute(RequestAttribute.ORG_NAME, orgName);
            req.setAttribute(RequestAttribute.ADMIN_FRAGMENT, RequestAttribute.FRAGMENT_ADD_ORG_CONFIRM);
